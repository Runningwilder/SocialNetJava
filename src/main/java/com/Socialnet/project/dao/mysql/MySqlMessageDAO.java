...
public class MySqlMessageDAO extends GenericDAO<Message> implements IMessageDAO {
	...
	public List<Message> findByFields(int fromUserId, int toUserId) throws SQLException {
		return findByFields(daoFactory.getConnection(), "SELECT * FROM Messages WHERE from_id = ? AND to_id = ?", fromUserId, toUserId);
	}

	@Override
	public void update(Message message) throws SQLException {
		updateByField(daoFactory.getConnection(),"UPDATE Messages SET time = ?, content = ?, from_id = ?, to_id = ? WHERE id = ?", message, 5, message.getId());
	}
